...
fig.patch.set_facecolor('#66000000')
ax1.set_facecolor('#66000000')
#plt.tight_layout()
...
aa[['Close']].plot(ax=ax1, legend=False, color='w', linewidth=4)
...
PP =   ( HighV + CloseV + LowV ) / 3
R1 =  ( 2 * PP ) - LowV
S1 =  ( 2 * PP ) - HighV
R2 =  PP + (HighV-LowV)
S2 =  PP - (HighV-LowV)

Percen = round(((TotalDel/PrevCloseV)*100),1)
...
ax1.axhline(highL, color='#19FF50', linestyle='dashed', linewidth=1, alpha=0.8)
ax1.axhline(LowL, color='#FF1919', linestyle='dashed', linewidth=1, alpha=0.8)
ax1.axhline(OlPP, color='#01FFFF', linestyle='dashed', linewidth=1, alpha=0.8)
ax1.axhline(ClPP, color='#EB15DC', linestyle='dashed', linewidth=1, alpha=0.8)
ax1.axhline(previousClose, color='#808080', linestyle='dashed', linewidth=1, alpha=0.8)

'''ax1.axhline(PP, color='#808080', linestyle='dotted', linewidth=2, alpha=0.8)
ax1.axhline(R1, color='#808080', linestyle='dotted', linewidth=2, alpha=0.8)
ax1.axhline(S1, color='#808080', linestyle='dotted', linewidth=2, alpha=0.8)
ax1.axhline(R2, color='#808080', linestyle='dotted', linewidth=2, alpha=0.8)
ax1.axhline(S2, color='#808080', linestyle='dotted', linewidth=2, alpha=0.8)'''
...
ax1.text(xHh, (highL+7), (str('High: ')+str(HighV)), bbox=dict(boxstyle = "square",facecolor = "#00FF00", edgecolor='#154734'), fontsize=20, fontweight="bold", color ="k")
ax1.text(xLl, (LowL-9), (str('Low: ')+str(LowV)), bbox=dict(boxstyle = "square",facecolor = "#FF0000", edgecolor='#800500'), fontsize=20, fontweight="bold")
ax1.text(open_min_10, (OlPP+0), (str('Open: ')+str(OpenV)), bbox=dict(boxstyle = "square",facecolor = "#FFFF00", edgecolor='#FF6600'), fontsize=20, fontweight="bold",horizontalalignment = "right", color ="k")
ax1.text(close_plus_10, (ClPP+0), (str('Close: ')+str(CloseV)), bbox=dict(boxstyle = "square",facecolor = "#CC00FF", edgecolor='#FF00FF'), fontsize=20, fontweight="bold",horizontalalignment = "left")
ax1.text(open_min_10, (previousClose - 3), (str('Prev.Close: ')+str(PrevCloseV)), bbox=dict(boxstyle = "square",facecolor = "#D7D2CB", edgecolor='#393D47'), fontsize=20, fontweight="bold",horizontalalignment = "right", color ="k")
ax1.axvline(xHh, color= '#FFFF00', linestyle='dotted', linewidth= .8, alpha = 0.9)
ax1.axvline(xLl, color= '#FFFF00', linestyle='dotted', linewidth= .8, alpha = 0.9)
...
'''
Titls = str("Nifty Bank: ") + str(today) + str(" | ") + str (CloseV) + str(" ( ")  + str (TotalDel) + str(" ) ")  + str(" ( ")  + str (Percen) + str(" %) ")
...
plt.savefig("04_ToDay_ChartB.png", facecolor=fig.get_facecolor())
...
Decri = ["Open", "Prev. Close", "GapOpen", "GapDir", "High", "Low", "High-Low", "Close", "Close Gap", "Close Dir", "Percen"]

Valus= []
Valus.append(OpenV)
Valus.append(PrevCloseV)
Valus.append(GapVV)
Valus.append(GapD)
Valus.append(HighV)
Valus.append(LowV)
Valus.append(HighLow)
Valus.append(CloseV)
Valus.append(TotalDelV)
Valus.append(CloseDir)
Valus.append(Percen)



datab = {'Name': Decri, 'Value': Valus}
dVL = pd.DataFrame(datab)
dVL.to_csv("00_DayValues.csv")
aa.to_csv("aaa.csv")
...
print ("Intraday Chart Data Generated.")