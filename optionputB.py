...
oi_data, ltp, crontime = oi_chain_builder("BANKNIFTY","latest","full")
#print(oi_data)
#print(ltp)
#print(crontime)
...
df.sort_values(by=['PUTS_OI'], inplace=True, ascending=False)
dff = df.head(3)
dff.to_csv("put.csv")
dft = pd.read_csv("put.csv")
for i in range (1, len(dft)):
    vlx.append(dft['PUTS_OI'][i])
...
vminn = aa['PUTS_OI'].min()
vmaxx = aa['PUTS_OI'].max()
vminn1 = aa['PUTS_Chng in OI'].min()
vmaxx1 = aa['PUTS_Chng in OI'].max()



bex1 = aa.index[aa['PUTS_OI']==vmaxx].tolist()
bex2 = aa.index[aa['PUTS_Chng in OI']==vminn1].tolist()
bex3 = aa.index[aa['PUTS_Chng in OI']==vmaxx1].tolist()
bex4 = aa.index[aa['PUTS_OI']==vlx[0]].tolist()
bex5 = aa.index[aa['PUTS_OI']==vlx[1]].tolist()
...
ListA.append(bex5[0])
ListB.append(vlx[1])
ListC.append(str("3rd Max. PUTS OI: ") +str(bex5[0]))
ListD.append("#06038D")
...
ListA.append(bex4[0])
ListB.append(vlx[0])
ListC.append(str("2nd Max. PUTS OI: ") +str(bex4[0]))
ListD.append("#06038D")

ListA.append(bex1[0])
ListB.append(vmaxx)
ListC.append(str("Max. PUTS OI: ") +str(bex1[0]))
ListD.append("#06038D")
...
ListA.append(bex2[0])
ListB.append(vminn1)
ListC.append(str("Max. PUTS unwinding: ") +str(bex2[0]))
ListD.append('#651C32')

ListA.append(bex3[0])
ListB.append(vmaxx1)
ListC.append(str("Max. PUTS writing: ") +str(bex3[0]))
ListD.append("#154734")
...
aa['PUTS_OI'].plot(ax =ax1, kind='line', color="#0827F5", linestyle='solid',linewidth= .4,)
x_axis = aa.index.get_level_values(0)
ax1.fill_between(x_axis, 0, aa['PUTS_OI'],color="#C710AF", alpha=.99,interpolate=False)
...
vminn = aa['PUTS_Chng in OI'].min()
vmaxx = aa['PUTS_Chng in OI'].max()
aa['PUTS_Chng in OI'].plot(ax =ax1, kind='line', color="k", linestyle='dotted',linewidth= .4)
ax1.fill_between(x_axis, 0, aa['PUTS_Chng in OI'], where=(aa['PUTS_Chng in OI'] <= 0), color='#FF033E', alpha=.9, interpolate=True)
ax1.fill_between(x_axis, 0, aa['PUTS_Chng in OI'], where=(aa['PUTS_Chng in OI'] > 0), color='#08FF08', alpha=.9, interpolate=True)
...
Titls = str("Bank Nifty Open Intrest PUTS  : ") + str(today)
...
filename =  str("08_Puts_Nifty")+str('.png')
...
#print (dfo)
dfo.to_csv("00_OptionValuesPUTS.csv")

print ("PUTS Option chain Data Generated.")
